...
def db(tmp_path):
    with ThermoDB(tmp_path / "test.db") as db:
        ...
        db.add_element(
            1, "H", standard_state="1/2 H2(g)",
            dfH0_298K=218.0, h298_minus_h0_atom=6.197, h298_minus_h0_std_state=8.468,
        )
        # He: monatomic standard state, master's own 0K value known (0).
        db.add_element(
            2, "He", standard_state="He(g)",
            dfH0_298K=0.0, h298_minus_h0_atom=6.197, h298_minus_h0_std_state=6.197,
        )
        ...
        db.add_element(
            6, "C", standard_state="C(s,gr)",
            dfH0_298K=716.67, h298_minus_h0_atom=6.536, h298_minus_h0_std_state=1.05,
        )
...
def test_does_not_overwrite_existing_by_default(tmp_path):
    with ThermoDB(tmp_path / "test2.db") as db:
        db.add_element(
            1, "H", dfH0_0K=999.0,  # a deliberately "wrong" existing value
            dfH0_298K=218.0, h298_minus_h0_atom=6.197, h298_minus_h0_std_state=8.468,
        )
        filled = derive_dfH0_0K(db)
